...
## Backend model
Backends provide the numerical implementation behind tensor operations. CPU is the reference behavior. CUDA implementations must match the same semantic contract within normal floating-point tolerance. As of Milestone 8, a real CUDA backend exists for a small forward-only operation set -- see `docs/architecture/cuda-backend.md`. As of Milestone 9, that operation set is reachable through `nn.Module` (`Module.to(device)`, then an ordinary `Linear`/`ReLU` forward pass), not just raw `Tensor` code -- see `docs/architecture/modules.md`.
