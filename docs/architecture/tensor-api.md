...
Operations: `+`, `-`, `*`, `/` (broadcasting elementwise ops -- `/` added M53, CUDA-capable but exact-shape-only there, see `docs/architecture/cuda-backend.md`), `@` (matmul, 1D/2D operands only), `.sum(axis=None, keepdims=False)` (CPU: any axis; CUDA, as of M14: `axis=None` or `axis=1`/`-1` on a 2D tensor only, see `docs/architecture/cuda-backend.md`), `.reshape(*shape)`, `.relu()` (M3, elementwise `max(x, 0)`), `.exp()`, `.log()` (M4, elementwise; added for a numerically stable `CrossEntropyLoss`, see `docs/architecture/optimization.md`; CUDA-capable as of M14), `.tanh()` (M50, elementwise; added for `nn.RNNCell`'s recurrence -- CUDA-capable, see `docs/architecture/cuda-backend.md`), `.sigmoid()` (M67, elementwise `1/(1+exp(-x))`, numerically stable per-sign branch; added for `nn.LSTMCell`'s input/forget/output gates -- CUDA-capable, see `docs/development/m67-lstm-long-range-recall.md`), `.sqrt()` (M53, elementwise; added for `nn.BatchNorm2d`'s `sqrt(var + eps)` -- CUDA-capable), `.batch_norm2d(...)` (M53, fused, **CUDA-only**; `nn.BatchNorm2d`'s CUDA call site -- the CPU path composes the equivalent math from `sum`/`sqrt`/`/`/`reshape` instead, see `docs/development/m53-batchnorm.md`), `.embedding_lookup(indices)` (M54, fused, CPU **and** CUDA; a `(vocab_size, embedding_dim)` table row-select by an int64 `indices` Tensor of any shape -- `nn.Embedding`'s one call site, added because a one-hot-plus-Linear workaround's compute cost scales with vocabulary size while a real lookup does not, measured at 40x-3500x across a 30-to-20,000-token vocabulary range, see `docs/development/m54-product-direction.md`), `.rnn_cell(w_ih, b_ih, w_hh, h)` (M55, fused, **CUDA-only**; `tanh(self @ w_ih + b_ih + h @ w_hh)` -- `nn.RNNCell`'s CUDA call site, added because a real char-RNN/word-RNN training step calls this composition once per timestep and the composed `Linear`/`+`/`.tanh()` path's per-launch cost (compounded by the CUDA default stream's per-kernel-launch synchronize) made CUDA training those workloads 1.5x-7.3x slower than CPU; CPU composes the equivalent computation from general ops unchanged, see `docs/development/m55-post-m54-assessment.md`), `.upsample_nearest2d(scale_factor)` (M63, CPU **and** CUDA; nearest-neighbor upsample of an NCHW tensor by an integer `(height, width)` scale factor -- `nn.UpsampleNearest2d`'s call site, added because no existing primitive could grow a spatial feature map back up (the shape-inverse of `max_pool2d`'s shrink), needed by `examples/autoencoder/`'s decoder; a real dedicated CUDA kernel pair, not composed from general broadcasting, since general N-D CUDA broadcast remains out of scope -- see `docs/development/m63-conv-autoencoder.md`), `.to(device)` (M8, explicit device transfer, see `docs/architecture/cuda-backend.md`), `.conv2d(weight, bias, stride, padding)` (M15, 2D cross-correlation over an NCHW input, CPU **and** CUDA -- `nn.Conv2d`'s call site; `nn.Conv1d` (M62) reshapes to a dummy `(N,C,1,L)` and calls this same primitive rather than adding a dedicated 1D kernel), `.max_pool2d(kernel_size, stride, padding)` (M15, 2D max pooling over an NCHW input, CPU **and** CUDA, deterministic first-maximum tie-break -- `nn.MaxPool2d`'s call site; `nn.MaxPool1d` (M62) reuses it the same way `Conv1d` reuses `.conv2d()`), `.cross_entropy(target)` (M31, fused mean cross-entropy over `(batch, classes)` logits and integer `(batch,)` class-index targets, CPU **and** CUDA -- `nn.CrossEntropyLoss`'s call site after its own validation, replacing a composed log-softmax chain for launch-count reasons, see `docs/performance/pipeline-profiling.md`). All operations return `Tensor`, never a raw NumPy array, and are differentiable when at least one operand requires grad. As of M10, CUDA operations this backend implements build and differentiate a real CUDA graph too (no longer forward-only); an operation a given device's backend doesn't implement (e.g. `sum(axis=0)` on CUDA) raises `CUDAError`/`UnsupportedDeviceError` as appropriate rather than building a graph it cannot correctly back-propagate through.
