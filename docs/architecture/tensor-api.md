...
Operations: `+`, `-`, `*` (broadcasting elementwise ops), `@` (matmul, 1D/2D operands only), `.sum(axis=None, keepdims=False)`, `.reshape(*shape)`. All operations return `Tensor`, never a raw NumPy array, and are differentiable when at least one operand requires grad.
...
## Not yet implemented
No neural-network modules, parameters, optimizers, losses, or CUDA execution. See `docs/development/roadmap.md`.
