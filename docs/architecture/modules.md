...
### `RNNCell`
`forge/nn/rnn.py`. One vanilla (Elman) RNN recurrence step: `h' = tanh(x @ W_ih + b_ih + h @ W_hh)`, composed from two existing `Linear` layers (`i2h`, with bias; `h2h`, without -- a second bias would be redundant, since both feed the same sum before `tanh`) and the new `Tensor.tanh()`. On CPU, `RNNCell` still has no forward/backward math of its own -- correctness follows entirely from `Linear` and `.tanh()`. As of Milestone 55, its CUDA path dispatches instead to a dedicated fused primitive (`Tensor.rnn_cell()`); see `docs/architecture/cuda-backend.md`'s **CUDA fused RNNCell** section for why (real char-RNN/word-RNN training calls this once per timestep, and the composed path's per-launch cost made CUDA training those workloads slower than CPU).
