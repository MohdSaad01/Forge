...
| [`image_folder_classification/`](image_folder_classification/README.md) | Classifying ordinary image *files on disk* (a generated circle/square/triangle shape dataset) via a directory-per-class layout -- the first example whose data source isn't a bundled format or in-memory array. | CNN (`Conv2d`/`BatchNorm2d`/`MaxPool2d`/`Dropout`) | + `data.ImageFolder`/`IMAGE_EXTENSIONS` (Milestone 69), `data.random_split`, `training.predict()` | CPU and CUDA (hardware-verified) |
