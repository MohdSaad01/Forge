...
| [`resnet/`](resnet/README.md) | A small ResNet-style image classifier with genuine branch + shortcut residual connections on MNIST -- (Milestone 66) asked whether Forge could express and train a residual network with the existing public API alone, and found no framework gap: zero changes to `forge/`. | Residual CNN (`Conv2d`/`BatchNorm2d`/`ReLU` blocks, one example-local `ResidualBlock` `Module`) | Same as `mnist/` (`Trainer`, `Adam`, checkpoint/resume, persistence), reuses `mnist/`'s dataset and `regression/`'s Milestone 65 reproducible-training workflow | CPU and CUDA (hardware-verified) |
...
| [`tabular_diabetes/`](tabular_diabetes/README.md) | Tabular classification on a real, externally-sourced dataset (the Pima Indians Diabetes CSV, 768 patients) rather than synthetic data -- (Milestone 92) found that a sentinel-coded missing value (`0` standing in for "not measured" in several columns) trains fine with `Normalize` alone but is silently lost from the saved artifact's preprocessing, giving a fresh raw inference row a materially different prediction depending on whether the developer manually reimplements imputation; fixed with the new persistable `data.transforms.ReplaceValue`, composed with `Normalize` as one saved `preprocessing=` pipeline; (Milestone 102) `app.py` demonstrates multi-row inference through one `forge.load_predictor()` call instead of reloading the artifact per row. | MLP (`Linear`/`ReLU` stack) | Same as `tabular_classification/` (`Trainer`, `Adam`, `data.random_split`, persistence, `task="tabular_classification"`) plus `data.transforms.ReplaceValue` (Milestone 92), `training.load_predictor()`/`ArtifactPredictor` (Milestone 102) | CPU and CUDA (hardware-verified) |
