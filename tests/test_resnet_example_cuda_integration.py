"""Milestone 66 integration tests: the `examples/resnet` residual-CNN pipeline on CUDA.

Mirrors `tests/test_mnist_example_cuda_integration.py` (same tiny synthetic
`(N, 1, 28, 28)` dataset, no real MNIST download) but drives
`Trainer(..., device="cuda")` against the nested custom `ResNetMNIST`/
`ResidualBlock` module tree, additionally verifying CUDA residency of
parameters/gradients/Adam state and CPU/CUDA numerical parity for a
residual forward pass. Skips cleanly when CUDA is unavailable;
...
from examples.resnet.model import ResidualBlock, build_model  # noqa: E402
...
def test_cuda_residency_of_parameters_gradients_and_adam_state():
    ...
    # BatchNorm2d buffers, nested three levels deep (model -> block -> bn), stay CUDA-resident too.
    for name, buf in model.named_buffers():
        assert isinstance(buf._data, CUDAStorage), f"buffer '{name}' is not CUDA-resident"
...
def test_checkpoint_save_and_resume_on_cuda(tmp_path):
    ...
    checkpoint_path = tmp_path / "resnet_tiny_cuda.ckpt"
    ...
    for _, buf in checkpoint.model.named_buffers():
        assert isinstance(buf._data, CUDAStorage)
...
def test_model_persistence_preserves_predictions_on_cuda(tmp_path):
    ...
    model_path = tmp_path / "resnet_tiny_cuda.forge"
...
def test_cpu_and_cuda_forward_agree_on_a_fresh_residual_block():
    """A residual block with a projection shortcut, moved from CPU to CUDA
    via `Module.to()` (so both devices share identical weights by
    construction), must produce numerically close output -- CPU/CUDA parity
    for the exact addition + BatchNorm2d composition this milestone
    introduces."""
    forge.random.seed(7)
    block = ResidualBlock(4, 8, stride=2, device="cpu")
    block.eval()

    x_np = np.random.default_rng(8).standard_normal((2, 4, 8, 8)).astype(np.float32)
    with no_grad():
        cpu_out = block(Tensor(x_np, device="cpu")).numpy().copy()

    block.to("cuda")
    with no_grad():
        cuda_out = block(Tensor(x_np, device="cuda")).to("cpu").numpy()

    np.testing.assert_allclose(cpu_out, cuda_out, atol=1e-4, rtol=1e-4)
