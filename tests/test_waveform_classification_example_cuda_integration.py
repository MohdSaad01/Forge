"""Milestone 62 integration tests: the `examples/waveform_classification` pipeline on CUDA.

Mirrors `tests/test_waveform_classification_example_integration.py` (same
small synthetic dataset) but drives `Trainer(..., device="cuda")`,
additionally verifying CUDA residency of parameters/gradients/Adam state and
CPU/CUDA prediction parity -- the acceptance criteria specific to this
milestone's CUDA path for `nn.Conv1d`/`nn.MaxPool1d`. Skips cleanly when CUDA
is unavailable; hardware-verified on the development machine's GeForce
940MX (CC 5.0) per `docs/development/development-environment.md`.
"""
...
from examples.waveform_classification.dataset import LENGTH, make_datasets  # noqa: E402
...
def test_checkpoint_save_and_resume_on_cuda(tmp_path):
    ...
    checkpoint_path = tmp_path / "waveform_tiny_cuda.ckpt"
...
def test_model_persistence_preserves_predictions_on_cuda(tmp_path):
    ...
    query = Tensor(np.random.default_rng(22).standard_normal((4, 1, LENGTH)).astype(np.float32), device="cuda")
    with forge.no_grad():
        pre_save = model(query).to("cpu").numpy()

    model_path = tmp_path / "waveform_tiny_cuda.forge"
...
def test_cpu_and_cuda_prediction_parity_after_training():
    ...
    query = Tensor(np.random.default_rng(99).standard_normal((8, 1, LENGTH)).astype(np.float32))
    ...
    def run(device: str) -> np.ndarray:
        ...
        optimizer = Adam(model.parameters(), lr=1e-3)
        trainer = Trainer(model, CrossEntropyLoss(), optimizer, device=device, verbose=False)
        trainer.fit(loader, epochs=1)
        with forge.no_grad():
